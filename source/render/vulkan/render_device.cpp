...
{
    const bool bVolumeTex = (desc->Flags >> 0xb & 1) == 0;
    ...
    imgCreateInfo.extent.depth = desc->Depth;
    imgCreateInfo.mipLevels = desc->MipCount;
    imgCreateInfo.arrayLayers = desc->ArrayLength;
    imgCreateInfo.format = kViewFormatLUT[desc->Format];
